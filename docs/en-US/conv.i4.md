...
online version: https://docs.microsoft.com/en-us/dotnet/api/system.reflection.emit.opcodes.conv_i4
schema: 2.0.0
---
...
3.  If the conversion is successful, the resulting value is pushed onto the stack.
 ...
 No exceptions are ever thrown when using this field. See `conv.ovf.i4` and `conv.ovf.i4.un` for equivalent instructions that will throw an exception when the result type can not properly represent the result value.
